...
```{r}
sum(is.na(train.data2$Embarked))
```

```{r}
table(train.data2$Embarked)
```

```{r}
train.data2$Embarked[is.na(train.data2$Embarked)] = "S" ## Remplacement par la valeur la plus commune
```
...
```{r}
table(test.titanic$Survived)
155/(263+155)
```

```{r}
table(train.titanic$Survived)
342/(342+549)
```

```{r}
PassengerId <- test.titanic$PassengerId
FinalDf <- as.data.frame(PassengerId)

FinalDf$Survived <- test.titanic$Survived

FinalDf$Survived <- recode(FinalDf$Survived, "FALSE" = "0", "TRUE" = "1")

write.csv(FinalDf, "titanic_kaggle_submission.csv", row.names = FALSE)
```
