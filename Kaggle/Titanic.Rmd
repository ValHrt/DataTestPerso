...
train.data2 <- train.data2[,-7] # retirer colonne Ticket (pas d'intérêt, toutes les valeurs sont différentes)
train.data2 <- train.data2[,-4] # retirer colonne Age (conserve uniquement Class.Age)
train.data2 <- train.data2[,-6] # retirer colonne Fare (conserve uniquement Class.Fare)
...
(model <- randomForest(Survived~., data = train.titanic, ntree = 1000, mtry = 4, na.action = na.omit))
```
