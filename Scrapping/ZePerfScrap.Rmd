...
```{r}
urlzp = read_html("http://www.zeperfs.com/fiche4403-audi-a3-iii-2-0-tdi.htm") ## Appeler l'URL avec les données du véhicule souhaité
```
...
RajoutBMW120D <- RajoutBMW120D %>%
  separate(col = "RajoutBMW120D",
           into = paste0("Info", 1:6), sep = " ") ## Séparer le df en plusieurs colonnes

RajoutBMW120D$Modèle <-paste(RajoutBMW120D$Info2,RajoutBMW120D$Info3,RajoutBMW120D$Info4, sep=" ") ## Rassembler les colonnes pour le modèle
RajoutBMW120D$Info2 <- NULL
RajoutBMW120D$Info3 <- NULL
RajoutBMW120D$Info4 <- NULL

RajoutBMW120D$Puissance <-paste(RajoutBMW120D$Info5,RajoutBMW120D$Info6,sep=" ") ## Rassembler les colonnes pour la puissance
RajoutBMW120D$Info5 <- NULL
RajoutBMW120D$Info6 <- NULL
...
# Lier tous les data frame

```{r}
BMW120D2 <- cbind(RajoutBMW120D, PoidsBMW120D)
```

```{r}
AudiA3 <- cbind(BMW120D2, BMW120D)
```

```{r}
write.csv2(AudiA3, file = "AudiA3.csv")
```
